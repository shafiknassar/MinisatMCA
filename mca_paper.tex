...
\begin{document}
	...
	Now we can formally define SAT. The Boolean Satisfiability Problem is the problem of determining whether a given boolean formula, usually CNF, is satisfiable.
	...
	\begin{algorithm}[H]
		...
		\KwOut{SAT, UNSTAT}
		\While{(true)}{
			R = resolveAll($ \mathcal{F} $)\\
			\eIf{R $\cap$ $\mathcal{F}$ $\neq$ R } {
				 $\mathcal{F}$ = $\mathcal{F}$ $\cup$ 
			}{
				break
			}
		}
